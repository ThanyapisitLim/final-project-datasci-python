...
# --- filter cluster 3 ---
cluster_3 = df[df['Cluster'] == 3].copy()

print("==================================================")
print("             📌 การวิเคราะห์ Cluster 3            ")
print("==================================================")
print("\n📌 ขนาดกลุ่ม 3:", len(cluster_3))

# อัตราการออก
churn_rate = (cluster_3['Churn'] == 'Yes').mean() * 100
print(f"🔥 อัตราการออกของ Cluster 3: {churn_rate:.2f}%")

# --- การกระจายตัวของ Contract ---
contract_distribution = cluster_3['Contract'].value_counts()
contract_proportion = (contract_distribution / len(cluster_3)) * 100

print("\n--- 📊 การกระจายตัวของ Contract ใน Cluster 3.0 ---")
print(f"จำนวนลูกค้าทั้งหมดในกลุ่ม 3: {len(cluster_3)} ราย")
...
print(f"\n✅ Contract ที่ใช้มากที่สุดใน Cluster 3 คือ: **{most_common_contract}**")
...
print("\n--- 📊 สัดส่วนการใช้บริการใน Cluster 3.0 (เปอร์เซ็นต์) ---")
for f in factors_service_proportion:
    service_proportion = (cluster_3[f].value_counts() / len(cluster_3)) * 100
...
print("\n--- 🧑‍🤝‍👩 การกระจายตัวของ Demographics (Cluster 3) ---")
for f in demographic_factors:
    demographic_proportion = (cluster_3[f].value_counts(normalize=True) * 100).round(2)
...
print("\n--- 📈 ปัจจัยที่สัมพันธ์กับ churn (เฉพาะ cluster 3) ---")
for f in factors_churn:
    pivot = pd.crosstab(cluster_3[f], cluster_3['Churn'], normalize='index') * 100
    print(f"\n=== {f} (Churn Rate) ===")
    print(pivot)
...
for col in viz_factors:
    pivot = pd.crosstab(cluster_3[col], cluster_3['Churn'], normalize='index') * 100
    pivot.plot(kind='bar', stacked=True)

    plt.title(f"Churn Distribution by {col} (Cluster 3)")
    plt.xlabel(col)
...
plt.tight_layout()
plt.suptitle("📊 Summary of Cluster 3 Factors", fontsize=16, y=1.02)
plt.show()
